...
================================================
机制:    DrissionPage 驱动真实 Chrome 完成账号密码登录及 Cap 人机验证。
...
def _cap_click(page) -> bool:
    """点击 Cap 验证组件"""
    try:
        widget = page.ele('tag:cap-widget', timeout=5)
        if not widget:
            return False
        time.sleep(1)
        sr = widget.shadow_root
        if sr:
            # 在 shadow-root 中找到触发验证的按钮并点击
            target = sr.ele('css:.captcha-trigger')
            if target:
...
def _fill_and_submit(page) -> None:
    ...
    if not _cap_token(page):
        raise RuntimeError("Cap 验证未取到 token")
    ...
    for attempt in range(1, NH_LOGIN_RETRIES + 1):
        ...
        try:
            ...
            if not _wait_login_form(page):
                raise RuntimeError("登录表单未出现（疑似被拦截）")
